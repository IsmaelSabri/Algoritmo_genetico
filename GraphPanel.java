...
public class GraphPanel extends JPanel {
	...
	* */
	...
	public GraphPanel(List<Double> scores, List<Double> puntos) {
		this.scores = scores;
		this.puntos = puntos;
	}
	...
	public static void createAndShowGui(List<Double> puntos, int generaciones, int limite) {
		List<Double> scores = new ArrayList<>();
		Random random = new Random();
		int maxDataPoints = generaciones;
		int maxScore = limite;
		for (int i = 0; i < maxDataPoints; i++) {
			scores.add((double) random.nextDouble() * maxScore);
		}
		GraphPanel mainPanel = new GraphPanel(scores, puntos);
		mainPanel.setPreferredSize(new Dimension(800, 600));
		JFrame frame = new JFrame("Población inicial");
